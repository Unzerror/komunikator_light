...
$total = compact_array(query_to_array("SELECT count(*)-1 FROM playlists"));
if (!is_array($total["data"]))
    echo out(array("success" => false, "message" => $total));

$data = compact_array(query_to_array("SELECT playlist_id as id, playlist, in_use FROM playlists WHERE playlist_id!=1 " . get_sql_order_limit()));
