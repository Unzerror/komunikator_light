...
SELECT (
    /*
    (SELECT count(*) FROM groups) + (SELECT count(*) FROM extensions) + 2
    */
    (SELECT count(*) FROM groups) + (SELECT count(*) FROM extensions) + 1
) count
EOD;

$total = compact_array(query_to_array($sql));

if (!is_array($total["data"]))
    echo out(array("success" => false, "message" => $total));

// $const = array('attendant', 'voicemail');  - из списка исключили: Автосекретарь, Голосовая почта
$const = array('attendant');  // из списка исключили: Голосовая почта
